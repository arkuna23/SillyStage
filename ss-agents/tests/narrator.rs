...
fn scene_request(purpose: NarratorPurpose) -> NarratorRequest {
    NarratorRequest {
        purpose,
        previous_node: None,
        current_node: NarrativeNode::new(
            "dock",
            "Flooded Dock",
...
async fn narrate_stream_emits_text_deltas_and_done() {
    ...
    let mut stream = narrator
        .narrate_stream(scene_request(NarratorPurpose::DescribeScene))
        .await
...
async fn describe_transition_requires_previous_node() {
    ...
    let error = match narrator
        .narrate_stream(scene_request(NarratorPurpose::DescribeTransition))
        .await
    {
...
async fn narrator_prompt_includes_shared_history_but_not_private_memory() {
    ...
    let request = NarratorRequest {
        ...
        previous_node: Some(NarrativeNode::new(
            "market",
            "Night Market",
