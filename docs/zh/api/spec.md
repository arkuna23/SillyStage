...
如果当前还没有 global config，那么第一条 `llm_api` 创建成功后，后端会自动把这个
`api_id` 绑定到所有 agent 的默认选择。
...
- global config 现在允许为空
- 未初始化时，读取接口返回 `api_ids = null`
- 只有真正执行 agent 的接口才要求 global config 已配置
