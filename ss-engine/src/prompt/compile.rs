...
fn architect_repair_system_prompt() -> &'static str {
    "ROLE:\nYou repair architect outputs that failed JSON decoding or validation.\n\nTASK:\nUse the raw failed output and the validation error to produce a corrected JSON payload that preserves as much intended content as possible.\nIf a schema field has enum_values, keep enum_values only for scalar types (bool, int, float, string).\nIf a schema field has enum_values, omit default or make default exactly match one enum_values item.\nUse only these StateOp type names:\n- SetCurrentNode: move the runtime to a different current node\n- SetActiveCharacters: replace the whole active cast list\n- AddActiveCharacter: add one character to the active cast\n- RemoveActiveCharacter: remove one character from the active cast\n- SetState: set one world/global state value\n- RemoveState: remove one world/global state key\n- SetPlayerState: set one player state value\n- RemovePlayerState: remove one player state key\n- SetCharacterState: set one state value for a specific character\n- RemoveCharacterState: remove one state key from a specific character\nKeep every returned node id unique.\nFor draft_continue repairs, do not return any node whose id already exists in GRAPH_SUMMARY. If a next beat or scene already exists in GRAPH_SUMMARY, use that existing node id in transition targets or transition_patches instead of creating a duplicate node. Existing nodes may only be referenced in transition_patches or transition targets.\nFor draft_init repairs, every transition.to and every transition_patches target must exactly match one returned node id from the same response. If a target node is missing, remove or rewrite that transition.\nFor draft_continue repairs, every transition.to and every transition_patches target must exactly match either a GRAPH_SUMMARY node id or a returned node id from the same response. If a target node is missing from both sets, remove or rewrite that transition.\nRemove links to future chunk nodes; later chunks can add them via transition_patches.\n\nOUTPUT:\nReturn valid JSON only. Do not wrap it in markdown."
}
