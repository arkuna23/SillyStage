...
import type { SchemaPresetDefinition, SchemaPresetKind } from './schema-presets'

type SchemaPresetDialogProps = {
  creating: boolean
  existingSchemaIds: ReadonlySet<string>
  onConfirm: (presetKinds: SchemaPresetKind[]) => Promise<void> | void
  onOpenChange: (open: boolean) => void
  open: boolean
  presets: ReadonlyArray<SchemaPresetDefinition>
}
...
export function SchemaPresetDialog({
  creating,
  existingSchemaIds,
  onConfirm,
  onOpenChange,
  open,
  presets,
}: SchemaPresetDialogProps) {
  const { t } = useTranslation()
  const [selectedPresetKinds, setSelectedPresetKinds] = useState<SchemaPresetKind[]>([])

  const selectablePresets = useMemo(
    () => presets.filter((preset) => !existingSchemaIds.has(preset.schemaId)),
    [existingSchemaIds, presets],
  )
  ...
  function togglePreset(kind: SchemaPresetKind) {
    ...
    <Dialog onOpenChange={onOpenChange} open={open}>
      ...
      >
        <DialogHeader className="border-b border-[var(--color-border-subtle)]">
          <DialogTitle>{t('schemas.presetDialog.title')}</DialogTitle>
        </DialogHeader>

        <DialogBody className="max-h-[calc(92vh-12rem)] overflow-y-auto pt-6">
          <div className="space-y-5">
            <p className="text-sm leading-7 text-[var(--color-text-secondary)]">
              {t('schemas.presetDialog.description')}
            </p>
            ...
            {allExisting ? (
              <div className="rounded-[1.35rem] border border-dashed border-[var(--color-border-subtle)] bg-[var(--color-bg-elevated)] px-4 py-5 text-sm text-[var(--color-text-secondary)]">
                {t('schemas.presetDialog.allExisting')}
              </div>
            ) : null}

            <div className="grid gap-3">
              {presets.map((preset) => {
                const alreadyExists = existingSchemaIds.has(preset.schemaId)
                    ...
                    status={
                      alreadyExists ? (
                        <Badge className="normal-case px-3 py-1.5" variant="info">
                          {t('schemas.presetDialog.alreadyExists')}
                        </Badge>
                      ) : null
                    }
                  />
          ...
          <div className="flex flex-col-reverse gap-3 sm:ml-auto sm:flex-row sm:items-center">
            {!allExisting ? (
              <span className="text-sm text-[var(--color-text-muted)]">
                {t('schemas.presetDialog.selectedCount', { count: selectedPresetKinds.length })}
              </span>
            ) : null}
            <Button
            ...
            >
              {creating
                ? t('schemas.presetDialog.creating')
                : t('schemas.presetDialog.createSelected')}
            </Button>
          </div>
        </DialogFooter>
      </DialogContent>
