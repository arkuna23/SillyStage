...
function demoCharacterNeedsCoverSync(
  ...
  return Boolean(
    summary && definition.coverUrl && definition.coverFileName && !summary.cover_file_name,
  )
}
...
function CharacterQuickActions({ onDelete, onEdit }: { onDelete: () => void; onEdit: () => void }) {
  const { t } = useTranslation()
      ...
      >
        ...
        <CardHeader className="gap-1 px-4 pb-1.5 pt-3">
          <div className="space-y-0.5">
            <CardTitle className="text-[1.34rem] leading-tight">{summary.name}</CardTitle>
      ...
      <div className="grid gap-3.5 p-3 sm:grid-cols-[4.25rem_minmax(0,10.5rem)_minmax(0,1fr)_auto] sm:items-center">
        ...
        >
          <div className="relative flex items-center justify-center">
            <CharacterArtwork coverUrl={coverUrl} mode="list" name={summary.name} />
            {selectionMode ? (
          ...
          <div className="min-w-0 space-y-0.5">
            <CardTitle className="truncate text-[1.08rem] leading-tight">{summary.name}</CardTitle>
...
export function CharacterManagementPage() {
  ...
  const [folderDropTarget, setFolderDropTarget] = useState<string | '__unfiled__' | null>(null)
  const [folderDialogState, setFolderDialogState] = useState<FolderDialogState | null>(null)
  ...
  const folderDeleteCharacters = useMemo(
    () =>
      folderDeleteTarget === null
        ? []
        : characters.filter(
            (character) => normalizeCharacterFolder(character.folder) === folderDeleteTarget,
          ),
  ...
  const refreshCharacters = useCallback(
    async (signal?: AbortSignal) => {
      ...
      try {
        ...
        const availableIds = new Set(summaries.map((summary) => summary.character_id))
  ...
  useEffect(() => {
    ...
    setFolderRegistry((current) => {
      const merged = Array.from(new Set([...current, ...foldersFromCharacters])).sort(
  ...
  useEffect(() => {
    ...
    if (selectedCharacterId !== null && !availableIds.has(selectedCharacterId)) {
      setSelectedCharacterId(null)
    }

    if (editingCharacterId !== null && !availableIds.has(editingCharacterId)) {
  ...
  async function handleFolderDialogSubmit() {
    ...
    if (folderDialogState.mode === 'create' && availableFolders.includes(normalizedFolder)) {
      setNotice({
    ...
    if (folderDialogState.mode === 'create') {
      setFolderRegistry((current) => addCharacterFolderRegistryEntry(current, normalizedFolder))
      closeFolderEditor()
  ...
  function handleFolderEditorBlur(event: FocusEvent<HTMLInputElement>) {
    if (
      ...
      event.currentTarget.closest('[data-folder-editor-item]')?.contains(event.relatedTarget)
    ) {
      return
    }
  ...
  async function handleFolderDelete() {
    ...
    try {
      ...
      if (deletedIds.length > 0) {
        ...
        setSelectedCharacterIds((currentSelection) =>
          currentSelection.filter((characterId) => !deletedIds.includes(characterId)),
        )
        ...
        if (selectedCharacterId !== null && deletedIds.includes(selectedCharacterId)) {
          setSelectedCharacterId(null)
        }
      }
      ...
      setFolderRegistry((current) => removeCharacterFolderRegistryEntry(current, targetFolder))
  ...
  async function ensureActorSchemaId() {
    const actorPreset = buildSchemaPresetDefinitions(t).find((preset) => preset.kind === 'actor')
      ...
      for (const definition of demoCharacterDefinitions) {
        ...
        try {
          ...
          if (definition.coverUrl && definition.coverFileName) {
            const coverFile = await loadDemoCoverFile(definition.coverUrl, definition.coverFileName)
  ...
  function openFolderContextMenu(event: React.MouseEvent<HTMLElement>, folder: string) {
    event.preventDefault()
    event.stopPropagation()
    setContextMenu({
      folder,
  ...
  function handleFolderDrop(event: React.DragEvent<HTMLElement>, folder: string | '__unfiled__') {
    if (!draggedCharacterId) {
      return
    }

    event.preventDefault()
    void moveCharactersToFolder([draggedCharacterId], folder === '__unfiled__' ? '' : folder)
  }
  ...
  async function handleDeleteCharacters() {
    ...
    try {
      ...
      if (failedTargets.length === 0) {
        ...
        setNotice({
          message: getErrorMessage(firstDeleteError, t('characters.feedback.deleteFailed')),
          tone: 'error',
        })
      }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-6">
      <CharacterDetailsDialog
        coverUrl={selectedCharacter ? coverUrls[selectedCharacter.character_id] : undefined}
        exporting={
          selectedCharacter !== null && exportingCharacterId === selectedCharacter.character_id
        }
        deleting={
          isDeleting &&
      ...
      <InsertSampleDialog
        ...
        confirmDisabled={
          !demoCharacterDefinitions.some((definition) => {
        ...
        items={demoCharacterDefinitions.map((definition) => ({
          ...
          status: characters.some((character) => character.character_id === definition.characterId)
            ? 'existing'
            : 'new',
        }))}
                  ...
                  {selectionMode ? (
                    <>
                      ...
                      <IconButton
                        ...
                        onClick={() => {
                          setSelectedCharacterIds(
                            filteredCharacters.map((character) => character.character_id),
                          )
                        }}
                        size="md"
                        variant="secondary"
            ...
            <div className="flex h-full min-h-0">
              ...
              >
                        ...
                        .map((folder) =>
                          ...
                          folderDialogState.folder === folder ? (
                            <FolderTreeEditorItem
                              icon={
                                <FontAwesomeIcon
                                  icon={activeFolder === folder ? faFolderOpen : faFolder}
                                />
                              }
                              inputRef={folderInputRef}
                              key={folder}
                ...
                <button
                  ...
                  type="button"
                >
                  <FontAwesomeIcon icon={isExplorerOpen ? faChevronLeft : faChevronRight} />
                </button>
              </motion.div>
                ...
                <div className="space-y-5">
                  <div className="space-y-4 rounded-[1.6rem] border border-[var(--color-border-subtle)] bg-[var(--color-bg-elevated)] p-4">
                    <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto]">
                      ...
                      <Badge className="h-11 self-start px-4 py-3 normal-case" variant="subtle">
